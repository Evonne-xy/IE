jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_parallax_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.vw-parallax-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-parallax-plugin-install');
                button.addClass('button-primary vw-parallax-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-parallax-recommended-plugins .vw-parallax-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-parallax-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-parallax-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-parallax-recommended-plugins').css('display','none');
    });
